...
const { AdminModel, UserModel, DentistModal } = require("./db")
...
const dentistAuthMiddleware = async (req, res, next) => {
  ...
  if (token) {
    try {
      const { email, id } = jwt.verify(token, process.env.JWT_SECRET)
      const dentist = await DentistModal.findOne({ _id: id })
...
module.exports = { adminAuthMiddleware, patientAuthMiddleware, dentistAuthMiddleware }